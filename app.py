...
<head>
    ...
    <style>
        ...
        .hud-panel { margin-top: 8px; display: flex; justify-content: space-between; width: 768px; font-size: 13px; font-weight: bold; background: rgba(15, 15, 25, 0.95); padding: 8px 12px; border: 2px solid #555; box-sizing: border-box; border-radius: 4px; }
        ...
        #settingsModal, #pauseModal, #achievementsModal { display: none; z-index: 25; text-align: left; }
...
<div class="game-wrapper">
    ...
    <div id="entryScreen" class="modal">
    ...
    <div class="hud-panel">
        <div>ARROWS/SPACE/X/Z | SCORE: <span id="hudScore">0</span> | BEST: <span id="hudHighScore">0</span> | COINS: <span id="hudCoins">100</span> | ❤️ <span id="hudLives">3</span> | x<span id="hudMultiplier">1</span></div>
        <div>
...
<script>
    ...
    let score = 0, highScore = parseInt(localStorage.getItem('mario_highscore') || '0');
    ...
    // Player
    const player = {
        x: 64, y: 200, width: 32, height: 32, vx: 0, vy: 0,
        speed: 4.2, jumpPower: -12.5, gravity: 0.5, grounded: false,
        facing: 'right', canDoubleJump: false, dashCooldown: 0,
        poweredUp: false, powerTimer: 0, fireballCooldown: 0,
        hasFirePower: false, starPower: false, starTimer: 0
    };

    // Character & Skin
    let selectedChar = 'mario', currentSkin = 'classic';
    let unlockedSkins = { classic: true, fire: false, gold: false, dark: false, galaxy: false, rainbow: false };
    let currentTheme = 'classic';

    // Game Objects
    let platforms = [], enemies = [], coins = [], decorations = [], hazards = [],
        movingPlatforms = [], thwomps = [], fireBars = [], powerUps = [],
        particles = [], floatingTexts = [], fireballs = [];
    let bossActive = false, bossHp = 100, bossX = 0, bossY = 0;
    ...
    function startBGM() {
        ...
        musicInterval = setInterval(() => {
            if (!isPaused && gameStarted && !gameOver) {
                ...
                if (freq > 0) playNote(freq, 0.18, 'square');
    ...
    function startGame() {
        ...
        lives = 3; score = 0; currentWorld = 1; comboMultiplier = 1;
        document.getElementById('hudLives').innerText = lives;
        document.getElementById('hudMultiplier').innerText = comboMultiplier;
    ...
    function returnToMainMenu() {
        gameOver = false; gameStarted = false; isPaused = true;
        document.getElementById('gameOverScreen').style.display = 'none';
    ...
    function openAchievements() {
        ...
        achievements.forEach(ach => {
            ...
            div.className = 'achievement-item' + (ach.unlocked ? ' achievement-unlocked' : '');
            div.innerHTML = '<strong>' + (ach.unlocked ? '✅' : '🔒') + ' ' + ach.name + '</strong><br>' + ach.desc;
    ...
    function closeSettingsMenu() {
        ...
        if (gameStarted && !gameOver) document.getElementById('pauseModal').style.display = 'block';
    }

    function selectCharacter(char) {
        selectedChar = char;
        if (char === 'mario') { player.speed = 4.3; player.jumpPower = -12.5; }
        else if (char === 'luigi') { player.speed = 4.0; player.jumpPower = -14.5; }
        else if (char === 'peach') { player.speed = 3.8; player.jumpPower = -11.5; }
        else if (char === 'yoshi') { player.speed = 4.5; player.jumpPower = -12.0; }
    ...
    function updateCustomParam(param, val) {
        if (param === 'speed') { player.speed = parseFloat(val); document.getElementById('valSpeed').innerText = val; }
        else if (param === 'jump') { player.jumpPower = parseFloat(val); document.getElementById('valJump').innerText = val; }
        else if (param === 'grav') { player.gravity = parseFloat(val); document.getElementById('valGrav').innerText = val; }
        else if (param === 'theme') { currentTheme = val; }
    }
    ...
    function addQuestionBlock(x, y) {
        ...
        if (rand > 0.7) powerUps.push({ x: x + 8, y: y - 24, type: 'mushroom', collected: false, vy: 0 });
        else if (rand > 0.5 && currentWorld >= 2) powerUps.push({ x: x + 8, y: y - 24, type: 'fireflower', collected: false, vy: 0 });
        else coins.push({ x: x + 16, y: y - 24, radius: 9, collected: false });
    }
    ...
    function spawnParticles(x, y, color) {
        for (let i = 0; i < 8; i++) {
            particles.push({ x: x, y: y, vx: (Math.random() - 0.5) * 6, vy: (Math.random() - 0.7) * 6, color: color, life: 35 });
        }
    }
    ...
    function generateChunk() {
        if (score >= worldScoreThreshold * currentWorld && !bossActive && Math.random() > 0.4) {
            bossActive = true; bossHp = 100 + (currentWorld * 20);
            bossX = lastGeneratedX + 200; bossY = 300;
            addGround(lastGeneratedX, 600, 'ground');
            lastGeneratedX += 600;
            return;
        }
        const groundWidth = 700 + Math.random() * 350;
        ...
        if (pattern === 0) {
            addPipe(lastGeneratedX + 160, 60); addPipe(lastGeneratedX + 380, 90);
            addQuestionBlock(lastGeneratedX + 270, 250);
            addGoomba(lastGeneratedX + 240, 352);
            if (currentWorld >= 2) addKoopa(lastGeneratedX + 460, 352);
        } else if (pattern === 1) {
            addBrick(lastGeneratedX + 220, 260); addQuestionBlock(lastGeneratedX + 252, 260);
            ...
            thwomps.push({ x: lastGeneratedX + 300, y: 60, startY: 60, width: 40, height: 40, crushing: false });
            fireBars.push({ x: lastGeneratedX + 440, y: 300, angle: 0, length: 55, speed: 0.055 });
        } else if (pattern === 3) {
            movingPlatforms.push({ x: lastGeneratedX + 150, y: 260, width: 80, height: 16, minX: lastGeneratedX + 130, maxX: lastGeneratedX + 430, vx: 1.9 });
            ...
            if (currentWorld >= 2) addBulletBill(lastGeneratedX + 500, 280);
        } else if (pattern === 5) {
            for (let i = 0; i < 4; i++) addBrick(lastGeneratedX + 200 + (i*32), 352 - ((i+1)*32));
        ...
        const pitSize = 80 + Math.random() * 80;
        if (Math.random() > 0.2) hazards.push({ x: lastGeneratedX, y: 392, width: pitSize, height: 40, type: 'lava' });
        lastGeneratedX += pitSize;
    }

    function resetGameState() {
        cameraX = 0; lastGeneratedX = 0;
        platforms = []; enemies = []; coins = []; decorations = [];
        hazards = []; movingPlatforms = []; thwomps = []; fireBars = [];
        powerUps = []; particles = []; floatingTexts = []; fireballs = [];
        score = 0; bossActive = false; currentWorld = 1;
        addGround(0, 900, 'ground');
        lastGeneratedX = 900;
        generateChunk();
        player.x = 64; player.y = 200; player.vx = 0; player.vy = 0;
        player.dashCooldown = 0; player.poweredUp = false;
        player.powerTimer = 0; player.hasFirePower = false;
    ...
    function updateWorldDisplay() {
        document.getElementById('worldDisplay').innerText = '🌍 WORLD ' + currentWorld;
    }

    function shootFireball() {
        fireballs.push({ x: player.x + (player.facing === 'right' ? 32 : 0), y: player.y + 16, vx: player.facing === 'right' ? 6 : -6, vy: 0, life: 60 });
        player.fireballCooldown = 20;
        playSFX('jump');
    }
    ...
    function triggerGameOver() {
        ...
        if (player.poweredUp && !player.hasFirePower) {
            player.poweredUp = false; player.height = 32; player.y -= 16;
            spawnParticles(player.x + 16, player.y + 16, '#e74c3c');
            playSFX('damage');
        ...
        if (player.hasFirePower) {
            player.hasFirePower = false; player.poweredUp = false; player.height = 32;
            spawnParticles(player.x + 16, player.y + 16, '#e74c3c');
            playSFX('damage');
            addFloatingText(player.x, player.y - 10, "POWER LOST!", "#e74c3c");
            return;
        }
        lives--;
        document.getElementById('hudLives').innerText = lives;
        ...
        if (lives <= 0) {
            ...
            player.x = cameraX + 64; player.y = 200; player.vx = 0; player.vy = 0;
            player.poweredUp = false; player.hasFirePower = false; player.height = 32;
        }
    }
    ...
    function checkAchievements() {
        achievements.forEach(ach => {
            if (!ach.unlocked && ach.check()) {
                ach.unlocked = true;
                addFloatingText(player.x, player.y - 40, "🏅 " + ach.name, "#f1c40f");
                playSFX('powerup');
            }
        });
    }
    ...
    window.addEventListener("keydown", (e) => {
        keys[e.code] = true;
        if(["Space", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "KeyX", "KeyZ"].includes(e.code)) e.preventDefault();
        if (e.code === "KeyX") triggerActiveSkill();
        if (e.code === "KeyZ" && player.hasFirePower && player.fireballCooldown <= 0) shootFireball();
    });
    ...
    function update() {
        ...
        // World Progression
        if (score >= worldScoreThreshold * currentWorld) {
            currentWorld++; updateWorldDisplay();
            addFloatingText(player.x, player.y - 60, "🌍 WORLD " + currentWorld + "!", "#f1c40f");
            playSFX('powerup');
        }
        ...
        platforms.forEach(platform => {
            if (player.x < platform.x + platform.width && player.x + player.width > platform.x &&
                player.y + player.height >= platform.y && player.y + player.height - player.vy <= platform.y + 14 && player.vy >= 0) {
                player.y = platform.y - player.height; player.vy = 0; player.grounded = true; player.canDoubleJump = true;
        ...
        movingPlatforms.forEach(mp => {
            ...
            if (player.x < mp.x + mp.width && player.x + player.width > mp.x &&
                player.y + player.height >= mp.y && player.y + player.height - player.vy <= mp.y + 12 && player.vy >= 0) {
                player.y = mp.y - player.height; player.vy = 0; player.grounded = true; player.canDoubleJump = true; player.x += mp.vx;
            }
        });

        // Jump
        if (keys["ArrowUp"] || keys["Space"]) {
            if (player.grounded) { player.vy = player.jumpPower; player.grounded = false; spawnParticles(player.x + 16, player.y + 32, '#fff'); playSFX('jump'); }
            else if (selectedChar === 'yoshi' && player.canDoubleJump) { player.vy = player.jumpPower * 0.9; player.canDoubleJump = false; spawnParticles(player.x + 16, player.y + 16, '#2ecc71'); playSFX('jump'); }
        }

        // Fireballs
        fireballs.forEach((fb, index) => {
            fb.x += fb.vx; fb.life--;
            if (fb.life <= 0) { fireballs.splice(index, 1); return; }
            enemies.forEach(enemy => {
                if (enemy.alive && fb.x < enemy.x + enemy.width && fb.x + 8 > enemy.x && fb.y < enemy.y + enemy.height && fb.y + 8 > enemy.y) {
                    enemy.alive = false;
                    score += 200 * comboMultiplier; coinsCollected += 1;
                    comboTimer = 120; comboMultiplier = Math.min(comboMultiplier + 1, 10);
                    document.getElementById('hudMultiplier').innerText = comboMultiplier;
        ...
        thwomps.forEach(t => {
            ...
            if (t.crushing) { t.y += 8; if (t.y >= 340) t.y = 340; setTimeout(() => { t.crushing = false; }, 700); }
            else if (t.y > t.startY) t.y -= 3;
            if (player.x < t.x + t.width && player.x + player.width > t.x && player.y < t.y + t.height && player.y + player.height > t.y)
                if (!player.starPower) triggerGameOver();
        });

        // Fire Bars
        fireBars.forEach(fb => {
            fb.angle += fb.speed;
            const tipX = fb.x + Math.cos(fb.angle) * fb.length;
            const tipY = fb.y + Math.sin(fb.angle) * fb.length;
        ...
        powerUps.forEach(pw => {
            if (!pw.collected) {
                pw.vy += player.gravity; pw.y += pw.vy;
                platforms.forEach(platform => {
                    if (pw.x < platform.x + platform.width && pw.x + 20 > platform.x && pw.y + 20 >= platform.y && pw.y + 20 - pw.vy <= platform.y + 14 && pw.vy >= 0) {
                        pw.y = platform.y - 20; pw.vy = 0;
                    }
                });
                const dist = Math.hypot((pw.x + 10) - (player.x + player.width / 2), (pw.y + 10) - (player.y + player.height / 2));
                if (dist < 24) {
                    pw.collected = true;
                    if (pw.type === 'mushroom') {
                        player.poweredUp = true; player.height = 48; player.powerTimer = 500;
                        score += 500; addFloatingText(player.x, player.y - 20, "+500 SUPER MUSHROOM!", "#2ecc71");
                        playSFX('powerup');
                    } else if (pw.type === 'fireflower') {
                        player.poweredUp = true; player.hasFirePower = true; player.height = 48; player.powerTimer = 500;
                        score += 500; addFloatingText(player.x, player.y - 20, "+500 FIRE FLOWER!", "#e74c3c");
                        playSFX('powerup');
                    }
        ...
        enemies.forEach(enemy => {
            ...
            enemy.vy += player.gravity; enemy.y += enemy.vy;
            platforms.forEach(platform => {
                if (enemy.x < platform.x + platform.width && enemy.x + enemy.width > platform.x &&
                    enemy.y + enemy.height >= platform.y && enemy.y + enemy.height - enemy.vy <= platform.y + 14 && enemy.vy >= 0) {
                    enemy.y = platform.y - enemy.height; enemy.vy = 0;
                }
            });
            if (enemy.type === 'bullet') { enemy.x += enemy.vx; if (enemy.x < cameraX - 100) enemy.alive = false; }
            else enemy.x += enemy.vx;
            if (player.x < enemy.x + enemy.width && player.x + player.width > enemy.x && player.y < enemy.y + enemy.height && player.y + player.height > enemy.y) {
                if (player.starPower) {
                    enemy.alive = false; score += 200 * comboMultiplier; coinsCollected += 1;
                    comboTimer = 120; comboMultiplier = Math.min(comboMultiplier + 1, 10);
                    document.getElementById('hudMultiplier').innerText = comboMultiplier;
                    addFloatingText(enemy.x, enemy.y - 15, "+200", "#f1c40f");
                    spawnParticles(enemy.x + 16, enemy.y + 16, '#f1c40f');
                    playSFX('stomp');
                } else if (player.vy > 0 && player.y + player.height - player.vy <= enemy.y + 14) {
                    enemy.alive = false; player.vy = -10;
                    score += 200 * comboMultiplier; coinsCollected += 1;
                    comboTimer = 120; comboMultiplier = Math.min(comboMultiplier + 1, 10);
                    document.getElementById('hudMultiplier').innerText = comboMultiplier;
                    addFloatingText(enemy.x, enemy.y - 15, "+200", "#f1c40f");
        ...
        coins.forEach(coin => {
            if (!coin.collected) {
                ...
                if (dist < coin.radius + player.width / 3) {
                    coin.collected = true;
                    score += 250 * comboMultiplier; coinsCollected += 1;
                    comboTimer = 120; comboMultiplier = Math.min(comboMultiplier + 1, 10);
                    document.getElementById('hudMultiplier').innerText = comboMultiplier;
        ...
        if (platforms.length > 80 && platforms[0].x < cameraX - 1000) {
            ...
            fireballs = fireballs.filter(fb => fb.x > cameraX - 800);
        }
    ...
    function drawPlayer(x, y, facing, h) {
        let shirtColor = '#e74c3c', overallColor = '#2980b9', hatColor = '#c0392b', skinTone = '#ffdbac';
        if (selectedChar === 'luigi') { shirtColor = '#2ecc71'; hatColor = '#27ae60'; }
        else if (selectedChar === 'peach') { shirtColor = '#f39c12'; overallColor = '#e74c3c'; hatColor = '#f1c40f'; }
        else if (selectedChar === 'yoshi') { shirtColor = '#27ae60'; overallColor = '#ffffff'; hatColor = '#2ecc71'; }
        if (player.hasFirePower) { shirtColor = '#ffffff'; overallColor = '#e74c3c'; hatColor = '#e74c3c'; }
        if (player.starPower) { shirtColor = Math.random() > 0.5 ? '#f1c40f' : '#e74c3c'; overallColor = Math.random() > 0.5 ? '#2ecc71' : '#3498db'; }
        if (currentSkin === 'fire') { shirtColor = '#ffffff'; overallColor = '#e74c3c'; hatColor = '#e74c3c'; }
        else if (currentSkin === 'gold') { shirtColor = '#f1c40f'; overallColor = '#d4ac0d'; hatColor = '#f1c40f'; }
        else if (currentSkin === 'dark') { shirtColor = '#34495e'; overallColor = '#111111'; hatColor = '#2c3e50'; }
        else if (currentSkin === 'galaxy') { shirtColor = '#8e44ad'; overallColor = '#2c3e50'; hatColor = '#9b59b6'; }
        else if (currentSkin === 'rainbow') { shirtColor = Math.random() > 0.5 ? '#e74c3c' : '#3498db'; overallColor = '#f1c40f'; hatColor = '#2ecc71'; }
        ctx.fillStyle = 'rgba(0,0,0,0.3)'; ctx.fillRect(x + 3, y + h - 2, 26, 4);
        ctx.fillStyle = hatColor; ctx.fillRect(x + (facing === 'right' ? 7 : 5), y, 22, 9);
        ctx.fillStyle = skinTone; ctx.fillRect(x + (facing === 'right' ? 11 : 5), y + 9, 16, 9);
        ctx.fillStyle = '#000'; ctx.fillRect(x + (facing === 'right' ? 19 : 7), y + 11, 3, 4); ctx.fillRect(x + (facing === 'right' ? 13 : 11), y + 15, 8, 3);
        ctx.fillStyle = shirtColor; ctx.fillRect(x + 5, y + 18, 22, h > 32 ? 20 : 10);
        ctx.fillStyle = overallColor; ctx.fillRect(x + 8, y + 22, 16, h > 32 ? 14 : 6);
        ctx.fillStyle = '#f1c40f'; ctx.fillRect(x + 9, y + 23, 3, 3); ctx.fillRect(x + 20, y + 23, 3, 3);
        ctx.fillStyle = '#4a2306'; ctx.fillRect(x + (facing === 'right' ? 16 : 2), y + h - 4, 14, 4);
    }
    ...
    function drawBoss(x, y) { ctx.fillStyle = '#b03a2e'; ctx.fillRect(x, y, 48, 48); ctx.fillStyle = '#f1c40f'; ctx.fillRect(x + 6, y + 10, 10, 10); ctx.fillRect(x + 32, y + 10, 10, 10); ctx.fillStyle = '#ffffff'; ctx.fillRect(x + 12, y + 36, 24, 6); ctx.fillStyle = '#000'; ctx.fillRect(x - 10, y - 16, 68, 8); ctx.fillStyle = '#2ecc71'; ctx.fillRect(x - 8, y - 14, (bossHp / (100 + currentWorld * 20)) * 64, 4); }
    ...
    function draw() {
        ...
        decorations.forEach(dec => {
            if (dec.type === 'cloud') { ctx.fillStyle = "rgba(255, 255, 255, 0.85)"; ctx.fillRect(dec.x, dec.y, 75, 22); ctx.fillRect(dec.x + 20, dec.y - 15, 35, 16); }
            else if (dec.type === 'bush') { ctx.fillStyle = currentTheme === 'neon' ? '#00ffcc' : '#1e8449'; ctx.fillRect(dec.x, dec.y, 95, 32); }
            else if (dec.type === 'castle') { ctx.fillStyle = '#566573'; ctx.fillRect(dec.x, dec.y - 30, 80, 62); }
        });

        platforms.forEach(platform => {
            if (platform.x + platform.width >= cameraX - 100 && platform.x <= cameraX + canvas.width + 100) {
                if (platform.type === 'ground') { ctx.fillStyle = currentTheme === 'midnight' ? '#1b4f72' : (currentTheme === 'neon' ? '#8e44ad' : '#a04000'); ctx.fillRect(platform.x, platform.y, platform.width, platform.height + 250); ctx.fillStyle = currentTheme === 'neon' ? '#00ffff' : '#27ae60'; ctx.fillRect(platform.x, platform.y, platform.width, 10); }
                else if (platform.type === 'ice') { ctx.fillStyle = '#5499c7'; ctx.fillRect(platform.x, platform.y, platform.width, platform.height + 250); }
                else if (platform.type === 'quicksand') { ctx.fillStyle = '#9a7d0a'; ctx.fillRect(platform.x, platform.y, platform.width, platform.height + 250); }
                else if (platform.type === 'brick') { ctx.fillStyle = '#b03a2e'; ctx.fillRect(platform.x, platform.y, platform.width, platform.height); }
                else if (platform.type === 'question') { ctx.fillStyle = '#d4ac0d'; ctx.fillRect(platform.x, platform.y, platform.width, platform.height); ctx.fillStyle = '#ffffff'; ctx.font = "bold 20px 'Courier New'"; ctx.fillText("?", platform.x + 9, platform.y + 24); }
                else if (platform.type === 'pipe') { ctx.fillStyle = '#27ae60'; ctx.fillRect(platform.x, platform.y, platform.width, platform.height); ctx.fillRect(platform.x - 4, platform.y, platform.width + 8, 16); }
            }
        });

        movingPlatforms.forEach(mp => { ctx.fillStyle = '#8e44ad'; ctx.fillRect(mp.x, mp.y, mp.width, mp.height); });
        hazards.forEach(h => { if (h.type === 'lava') { ctx.fillStyle = '#c0392b'; ctx.fillRect(h.x, h.y, h.width, h.height); } else if (h.type === 'spikes') { ctx.fillStyle = '#7f8c8d'; for (let sx = h.x; sx < h.x + h.width; sx += 16) { ctx.beginPath(); ctx.moveTo(sx, h.y + h.height); ctx.lineTo(sx + 8, h.y); ctx.lineTo(sx + 16, h.y + h.height); ctx.fill(); } } });
        fireBars.forEach(fb => { ctx.fillStyle = '#7f8c8d'; ctx.beginPath(); ctx.arc(fb.x, fb.y, 6, 0, Math.PI * 2); ctx.fill(); for (let r = 12; r <= fb.length; r += 14) { const px = fb.x + Math.cos(fb.angle) * r; const py = fb.y + Math.sin(fb.angle) * r; ctx.fillStyle = '#e67e22'; ctx.beginPath(); ctx.arc(px, py, 6, 0, Math.PI * 2); ctx.fill(); } });
        thwomps.forEach(t => { drawThwomp(t.x, t.y); });
        if (bossActive) drawBoss(bossX, bossY);
        enemies.forEach(enemy => { if (enemy.alive) { if (enemy.type === 'koopa') drawKoopa(enemy.x, enemy.y); else if (enemy.type === 'bullet') drawBulletBill(enemy.x, enemy.y); else drawGoomba(enemy.x, enemy.y); } });
        powerUps.forEach(pw => { if (!pw.collected) { if (pw.type === 'mushroom') { ctx.fillStyle = '#e74c3c'; ctx.fillRect(pw.x, pw.y, 20, 20); ctx.fillStyle = '#fff'; ctx.fillRect(pw.x + 4, pw.y + 4, 4, 4); ctx.fillRect(pw.x + 12, pw.y + 4, 4, 4); } else if (pw.type === 'fireflower') { ctx.fillStyle = '#e74c3c'; ctx.beginPath(); ctx.arc(pw.x + 10, pw.y + 10, 10, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#f1c40f'; ctx.beginPath(); ctx.arc(pw.x + 10, pw.y + 10, 5, 0, Math.PI * 2); ctx.fill(); } } });
        fireballs.forEach(fb => { ctx.fillStyle = '#e74c3c'; ctx.beginPath(); ctx.arc(fb.x + 4, fb.y + 4, 6, 0, Math.PI * 2); ctx.fill(); });
        coins.forEach(coin => { if (!coin.collected) { ctx.fillStyle = '#f1c40f'; ctx.beginPath(); ctx.arc(coin.x, coin.y, coin.radius, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#f39c12'; ctx.beginPath(); ctx.arc(coin.x, coin.y, coin.radius - 3, 0, Math.PI * 2); ctx.fill(); } });
        particles.forEach(p => { ctx.fillStyle = p.color; ctx.fillRect(p.x, p.y, 4, 4); });
        floatingTexts.forEach(ft => { ctx.fillStyle = ft.color; ctx.font = "bold 13px 'Courier New'"; ctx.fillText(ft.text, ft.x, ft.y); });
        if (!gameOver) drawPlayer(player.x, player.y, player.facing, player.height);
        ctx.restore();
    }
